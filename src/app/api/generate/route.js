...
export async function POST(req) {
  try {
    ...
    let actor = null;
    if (actorId) {
      actor = await prisma.actor.findFirst({ where: { id: actorId, userId: me.id } });
    ...
    const { apiKey, keySource } = resolveApiKey(provider, me.apiKeys);
    if (!keySource) {
    ...
    if (requiredCredits > 0) {
      const reserved = await prisma.user.updateMany({
        where: { id: me.id, credits: { gte: requiredCredits } },
        data: { credits: { decrement: requiredCredits } },
      });
      if (reserved.count === 0) {
        return NextResponse.json(
          { error: `Insufficient credits. This needs ${requiredCredits} credits but you have ${me.credits}.` },
          { status: 403 }
        );
      }
    }
    ...
    try {
      ...
      if (requiredCredits > 0) {
        await prisma.user.update({ where: { id: me.id }, data: { credits: { increment: requiredCredits } } });
      }
      throw err;
    }
